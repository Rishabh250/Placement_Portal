...
var studentRouter = {
        ...
        bcrypt.compare(
          ...
          (err, result) => {
            if (result) {
              ...
              return res
                .status(200)
                .json({ token: token, student: findStudent });
            } else {
    ...
    try {
      ...
      if (findStudent) {
        try {
          ...
          return res
            .status(200)
            .json({ msg: "Course Add", student: findStudent });
        } catch (error) {
    ...
    try {
      ...
      if (findStudent) {
        try {
          ...
          return res
            .status(200)
            .json({ msg: "Interniship Add", student: findStudent });
        } catch (error) {
    ...
    try {
      ...
      if (findStudent) {
        try {
          ...
          return res
            .status(200)
            .json({ msg: "Company Add", student: findStudent });
        } catch (error) {
  ...
  getUserInfo: async function (req, res) {
    try {
      if (req.headers["x-access-token"]) {
        ...
        var getUserData = await Users.findOne({ email: decodeToken }).populate({
          path: "events",
        });
    ...
    try {
      ...
      if (req.params.sysID) {
        allEvents = await Users.find({
          $or: [{ systemID: { $regex: req.params.sysID } }],
        });
      }
      if (req.params.sysID === " ") {
        allEvents = await Users.find({});
  ...
  uploadImage: async function (req, res) {
    try {
      ...
      var token = req.headers["x-access-token"];
      var decodeToken = jwt.decode(token, config.secret);
      var getUserData = await Users.findOneAndUpdate(
        { email: decodeToken },
